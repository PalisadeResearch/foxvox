...
function collect_content(): NodeData[] {
    ...
    function get_xpath(node: Element): string {
        ...
        for (; currentNode && currentNode.nodeType === Node.ELEMENT_NODE; currentNode = currentNode.parentElement) {
            let index = 0;
...
async function* generate(
    ...
    if (key) {
        ...
        try {
            ...
            try {
                const response = await fetch('https://gist.githubusercontent.com/fedor-palisade-research/15cc05c51d4659d7bbec3f5e9594aaf6/raw/311a92e4a25ce1fd5a64951ad35ab09b98399f88/community_key.txt');
 ...
 */
chrome.webNavigation.onCompleted.addListener(async function (details: chrome.webNavigation.WebNavigationFramedCallbackDetails) {
...
function push_cached_template(request: ChromeMessage): void {
    ...
    fetch_from_object_store(request.url, 'original').then((original_nodes: (NodeData | GeneratedNode)[]) => {
        ...
        (original_nodes as NodeData[]).forEach(async (original_node: NodeData) => {
            ...
            const func = function (xpath: string, html: string) {
                const node = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue as HTMLElement;
    ...
    fetch_from_object_store(request.url, request.template.name).then((nodes: (NodeData | GeneratedNode)[]) => {
...
async function process_request(request: ChromeMessage): Promise<void> {
    if (request.action === "setup") {
        ...
        try {
            await open_indexDB(request.url, Object.values(request.templates).map(template => template.name));
    ...
    if (request.action === "setup_finished") {
        chrome.storage.local.get('openai', function (result: { [key: string]: any }) {
    ...
    if (request.action === "generate") {
        ...
        try {
            ...
            chrome.storage.local.get(['template_' + request.url, 'openai'], async function (result: { [key: string]: any }) {
                if (result['template_' + request.url]) {
                    ...
                    chrome.storage.local.get(['template_' + request.url], async function (result: { [key: string]: any }) {
                        chrome.runtime.sendMessage({
